...
set -e
if grep -q "block aaaa record" server_aaaa.log; then
	echo -e "${GREEN}PASS${NC}"
	SUCCESS=$((SUCCESS + 1))
else
	echo -e "${RED}FAIL${NC}"
	echo "Expected log 'block aaaa record', got:"
	cat server_aaaa.log
	FAIL=$((FAIL + 1))
fi
kill $pid 2>/dev/null || true
...
set -e
if grep -q "local query" server_aaaa_allow.log && ! grep -q "block aaaa record" server_aaaa_allow.log; then
	echo -e "${GREEN}PASS${NC}"
	SUCCESS=$((SUCCESS + 1))
else
	echo -e "${RED}FAIL${NC}"
