...
func TestStressAndStability(t *testing.T) {
	...
	handler := func(w dns.ResponseWriter, r *dns.Msg) {
		...
		if len(r.Question) > 0 {
			...
			if q.Qtype == dns.TypeA {
				ip := fmt.Sprintf("%d.%d.%d.%d", rand.Intn(250)+1, rand.Intn(250)+1, rand.Intn(250)+1, rand.Intn(250)+1)
				rr, _ := dns.NewRR(fmt.Sprintf("%s 10 IN A %s", q.Name, ip))
				if rr != nil {
	...
	dummySrv := &dns.Server{PacketConn: pc, Handler: dns.HandlerFunc(handler)}
	go dummySrv.ActivateAndServe()
	defer dummySrv.Shutdown()
	...
	m := new(dns.Msg)
	m.SetQuestion("warmup.com.", dns.TypeA)
	client.Exchange(m, listenAddr)
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(routineID int) {
			...
			for j := 0; j < queriesPerRoutine; j++ {
				m := new(dns.Msg)
				m.SetQuestion(fmt.Sprintf("test-%d-%d.com.", routineID, j), dns.TypeA)
				r, _, err := client.Exchange(m, listenAddr)
