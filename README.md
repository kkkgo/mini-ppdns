...
- 去[release](https://github.com/kkkgo/mini-ppdns/tree/main/release)下载适合你的硬件架构的二进制文件并**解压**。如果不清楚自己的硬件是什么架构，可以在终端输入`uname -m`。其中release名字带UPX的是为了给一些储存空间紧张的设备用的，如果你的设备空间充足下正常版本即可。
- 把解压后的`mini-ppdns`上传到你的设备，为了方便你可以上传到`/usr/sbin/mini-ppdns`,加执行权限`chmod +x /usr/sbin/mini-ppdns`。将你的配置文件储存在`/etc/mini-ppdns.ini`，然后执行`mini-ppdns -config /etc/mini-ppdns.ini`看看是否输出正常（比如提示某个端口已经被监听）。
