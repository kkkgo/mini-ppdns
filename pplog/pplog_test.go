...
func TestEncodeHeader(t *testing.T) {
	...
	uuid := [16]byte{0x99, 0x0c, 0x7c, 0x49, 0xdb, 0xb2, 0x47, 0x0b,
		0xbb, 0x05, 0x2f, 0x82, 0x60, 0x28, 0x17, 0x59}

	n := EncodeHeader(buf[:], 2, 42, uuid, 48)
	if n != HeaderSize {
	...
	if buf[2] != 0x01 {
		t.Errorf("version = %02x, want 01", buf[2])
	}
	if buf[3] != 2 {
		t.Errorf("level = %d, want 2", buf[3])
	}
	...
	for i := 0; i < 16; i++ {
		if buf[8+i] != uuid[i] {
			t.Errorf("uuid[%d] = %02x, want %02x", i, buf[8+i], uuid[i])
		}
	}
	plen := binary.BigEndian.Uint16(buf[24:26])
	if plen != 48 {
		t.Errorf("payloadLen = %d, want 48", plen)
	}
}
...
func TestEncodeQueryEntry_Level2(t *testing.T) {
	...
	expectedLen := level1Len + 1 + len("udp://8.8.8.8:53") // 1 byte length prefix + 16 chars
	if n != expectedLen {
...
func TestReporter_ReceivesPacket(t *testing.T) {
	pc, err := net.ListenPacket("udp", "127.0.0.1:0")
	if err != nil {
		t.Fatalf("listen: %v", err)
	}
	...
	if buf[0] != MagicByte0 || buf[1] != MagicByte1 {
		t.Errorf("magic = %02x%02x, want %02x%02x", buf[0], buf[1], MagicByte0, MagicByte1)
	}
	if buf[2] != Version {
		t.Errorf("version = %d, want %d", buf[2], Version)
	}
	if buf[3] != 1 {
		t.Errorf("level = %d, want 1", buf[3])
	}

	plen := binary.BigEndian.Uint16(buf[24:26])
	if HeaderSize+int(plen) != n {
		t.Errorf("header(%d) + payload(%d) = %d, but received %d bytes", HeaderSize, plen, HeaderSize+int(plen), n)
	}
}
