...
func TestPackBuffer(t *testing.T) {
	...
	rr, _ := dns.New("example.com. 3600 IN A 1.2.3.4")
	...
	if len(m2.Question) == 0 || m2.Question[0].Header().Name != "example.com." {
...
func TestPackBuffer_EmptyMsg(t *testing.T) {
	m := new(dns.Msg)
	dnsutil.SetQuestion(m, "test.com.", dns.TypeAAAA)
...
func TestPackBuffer_ReleaseIsSafe(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		m := new(dns.Msg)
		dnsutil.SetQuestion(m, "test.com.", dns.TypeA)
		rr, _ := dns.New("test.com. 60 IN A 10.0.0.1")
		m.Answer = append(m.Answer, rr)
...
func BenchmarkPool_PackBuffer(b *testing.B) {
	base := new(dns.Msg)
	dnsutil.SetQuestion(base, "example.com.", dns.TypeA)
	for i := 0; i < 10; i++ {
		r, _ := dns.New("example.com. 3600 IN A 1.2.3.4")
		base.Answer = append(base.Answer, r)
	}

	b.ResetTimer()
	b.RunParallel(func(pb *testing.PB) {
		// PackBuffer mutates m.Data, so each goroutine needs its own copy.
		m := base.Copy()
		for pb.Next() {
			buf, err := PackBuffer(m)
			if err == nil {
...
func BenchmarkPool_PackTCPBuffer(b *testing.B) {
	...
	rand.Read(payload)
	r := new(dns.TXT)
	r.Hdr = dns.Header{Name: "example.com.", Class: dns.ClassINET, TTL: 3600}
	r.TXT = rdata.TXT{Txt: []string{string(payload)}}
	base.Answer = append(base.Answer, r)
