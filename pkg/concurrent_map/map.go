...
// Return a cache with a maximum size.
// Note that, because it has multiple (MapShardSize) shards,
// the actual maximum size is MapShardSize*(size / MapShardSize).
// If size <=0, equal to NewMap().
...
func (m *shard[K, V]) rangeReadOnly(f func(k K, v V) bool) bool {
	m.l.RLock()
	defer m.l.RUnlock()
	for k, v := range m.m {
		if !f(k, v) {
