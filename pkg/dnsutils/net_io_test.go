...
func TestUDPReadWrite(t *testing.T) {
	m := new(dns.Msg)
	dnsutil.SetQuestion(m, "udp.com.", dns.TypeMX)
	...
	if len(m2.Question) == 0 || m2.Question[0].Header().Name != "udp.com." {
		t.Fatal("question mismatch")
	}
}
...
func BenchmarkTCP_WriteRead(b *testing.B) {
	m := new(dns.Msg)
	dnsutil.SetQuestion(m, "bench.com.", dns.TypeA)
	rr, _ := dns.New("bench.com. 60 IN A 10.0.0.1")
	m.Answer = append(m.Answer, rr)
