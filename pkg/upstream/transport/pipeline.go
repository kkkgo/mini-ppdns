...
// Pipeline queries as RFC 7766 6.2.1.1 suggested.
// Also reuse udp socket. Since dns over udp is some kind of "pipeline".
...
type PipelineOpts struct {
	...
	// Note: If connection turns out having a smaller limit, part of queued up
	// queries will fail.
...
func (t *PipelineTransport) ExchangeContext(ctx context.Context, m []byte) (*[]byte, error) {
	...
	for {
		...
		if err != nil {
			...
			// Try to re-send this query if failed on a reused connection.
...
// Close PipelineTransport and all its connections.
