...
const (
	// Most servers send SERVFAIL after 3~5s. If no resp, connection may be dead.
	reuseConnQueryTimeout = time.Second * 6
)

// For old tcp protocol. (no pipelining)
...
type ReuseConnOpts struct {
	// Specifies the method to dial a connection to the server.
	// MUST NOT be nil.
	DialContext func(ctx context.Context) (NetConn, error)

	// Specifies the timeout for DialFunc.
...
// Dial a *reusableConn.
// Caller must call releaseReusableConn to release the reusableConn.
func (t *ReuseConnTransport) getNewConn(ctx context.Context) (*reusableConn, error) {
	...
	go func() {
		...
		if err != nil {
			t.logger.Warnf("fail to dial reusable conn err=%v", err)
			// The original code did not return here, it continued to the `if c != nil` block.
			// To maintain the original logic flow, we should not return early here.
			// The `return nil, false, err` in the instruction is not compatible with the `dialRes` struct.
		}
		if c != nil {
...
// Return a *reusableConn from conn pool, or nil if no conn
// is idle.
// Caller must call releaseReusableConn to release the reusableConn.
...
// Close ReuseConnTransport and all its connections.
...
func (c *reusableConn) readLoop() {
	for {
		...
		// Connection has idled again.
		c.c.SetReadDeadline(time.Now().Add(c.t.idleTimeout))
		// Note: calling setIdle before sending resp back to make sure connection is idle
...
func (c *reusableConn) exchange(ctx context.Context, q *[]byte) (*[]byte, error) {
	...
	// Drain any lagging payloads from the reusable channel from a previous timeout
	select {
	case oldResp := <-c.respChan:
		pool.ReleaseBuf(oldResp)
	default:
	}
